...
<script type="text/javascript">
	...
	function showAddCustomerDetail() {
		//getTaskNumber();
		
		$.get("${ctx}/account/addCustomerDetail.do", function(data) {
			changeURL("${ctx}/account/addCustomerDetail.do");
			$('.maincontent').html($(data).find(".maincontent").html());
			initProv("#province", "#city", "", "");
		});
	}
	
	function showAddEmployeeDetail() {
		//getTaskNumber();
		
		$.get("${ctx}/account/addEmployeeDetail.do", function(data) {
			changeURL("${ctx}/account/addEmployeeDetail.do");
			$('.maincontent').html($(data).find(".maincontent").html());
			initRole("#department", "#role", "市场部", "市场主管");
		});
