...
public class OrderMobileController {
	...
	@RequestMapping(value = "account/mobile_modifyOrderList.do", method = RequestMethod.GET)
	public void getOrderList(HttpServletRequest request,
		...
		model.addAttribute("url", "/account/mobile_modifyOrderDetail.do");
		model.addAttribute("searchurl", "/account/mobile_modifyOrderSearch.do");
		jsonUtil.sendJson(response, model);
	}
	
	@RequestMapping(value = "account/mobile_modifyOrderSearch.do")
...
//		Account account = (Account) session.getAttribute("cur_user");
	...
	@RequestMapping(value = "account/mobile_modifyOrderDetail.do")
	public void modifyOrderDetail(HttpServletRequest request,
		...
		if(orderModel == null){
			...
			model.addAttribute(Constants.JSON_NOTIFY, "该订单已经签订过大货合同，不可修改");
			jsonUtil.sendJson(response, model);
			return;
		}
	...
	// 询单的修改界面
		@RequestMapping(value = "account/mobile_modifyOrderSubmit.do", method = RequestMethod.POST)
		public void modifyOrderSubmit(HttpServletRequest request,
			...
			if(!account.getUserName().equals("admin")){
				model.addAttribute(Constants.JSON_IS_SUCCESS, false);
				model.addAttribute(Constants.JSON_NOTIFY, "非管理员，无修改权限！");
				jsonUtil.sendJson(response, model);
				return;
			}
...
//					: false;
		...
		// =======================被终止订单列表=================================
		@RequestMapping(value = "/order/mobile_endList.do")
		public void endList(HttpServletRequest request,
				HttpServletResponse response, ModelMap model) {
			Account account = commonService.getCurAccount(request, response);
			if (account == null) {
				return;
		...
		// =======================被终止订单列表搜索=================================
		@RequestMapping(value = "/order/mobile_endListSearch.do")
		public void endListSearch(HttpServletRequest request,
				HttpServletResponse response, ModelMap model) {
			Account account = commonService.getCurAccount(request, response);
			if (account == null) {
				return;
...
//			List<Map<String,Object>>list=orderService.findByProperty("orderState","1");
		...
		// ===========================结束订单=================================
		@RequestMapping(value = "/order/mobile_end.do")
		public void end(HttpServletRequest request,
				HttpServletResponse response, ModelMap modelt){
			Integer orderId=Integer.parseInt(request.getParameter("orderId"));
			orderService.endOrder(orderId);
			modelt.addAttribute(Constants.JSON_IS_SUCCESS, true);
			jsonUtil.sendJson(response, modelt);
		}
		
		// ==========================推送订单信息===============================
		@RequestMapping(value = "/order/mobile_pushOrderInfo.do")
		public void pushOrderInfo(HttpServletRequest request,
				HttpServletResponse response, ModelMap model){
			Integer orderId = Integer.parseInt(request.getParameter("orderId"));
